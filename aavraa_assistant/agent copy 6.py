...
from aavraa_assistant.instructions import (AAVRAA_INTRODUCTORY_AGENT_INSTRUCTION,PRICE_RESEARCHER_AGENT_INSTRUCTION,AAVRAA_SHOP_AGENT_INSTRUCTION,MARKET_RESEARCHER_AGENT_INSTRUCTION)
import os
import requests
import json
...
pricing_research_agent = Agent(
    model="gemini-2.0-flash-exp",
    name='aavraa_pricing_research_agent',
    description=('''
        A market price researcher for Aavraa. Receives a search queries from research agent
        and returns a list of the generated queries in English and there prices.
    '''),
    instruction=PRICE_RESEARCHER_AGENT_INSTRUCTION,
    tools=[google_search],
)
