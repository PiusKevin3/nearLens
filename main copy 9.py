...
async def start_agent_session(user_id: str, is_audio: bool = False):
    ...
    research_agent_runner = InMemoryRunner(
        app_name=APP_NAME,
    ...
    research_agent_run_config = RunConfig(response_modalities=["TEXT"])

    research_agent_live_request_queue = LiveRequestQueue()
    live_request_queue = LiveRequestQueue()


    research_agent_live_events = research_agent_runner.run_live(
        session=session,
        live_request_queue=live_request_queue,
        run_config=research_agent_run_config,
    )
    ...
    return live_events, live_request_queue,research_agent_live_events,research_agent_live_request_queue
...
async def audio_websocket_endpoint(websocket: WebSocket, user_id: int):
    ...
    live_events, live_request_queue,research_agent_live_events,research_agent_live_request_queue = await start_agent_session(str(user_id), is_audio=True)
    ...
    async def keepalive_ping():
        try:
            while True:
                ...
                print(f"[KEEPALIVE] Sent ping to audio client #{user_id}")
...
async def text_websocket_endpoint(websocket: WebSocket, user_id: int):
    ...
    async def keepalive_ping():
        try:
            while True:
                await websocket.send_text("__ping__")
                print(f"[KEEPALIVE] Sent ping to text client #{user_id}")
                await asyncio.sleep(10)
