...
# ===============================
# INPUT MODEL (moment insight + places request)
# ===============================
class NearbyPlaceRequest(BaseModel):
    # Required LLM insight fields
    text: str
    category: List[str]
    place_type: str
    keywords: List[str]
...
def find_nearby_places(req: NearbyPlaceRequest) -> Dict:
    ...
    try:
        ...
        for p in places[:8]:
            ...
            photo_url = (
                build_photo_url(photo_name, api_key)
            ...
            results.append({
                ...
                "types": ", ".join(
                    t.replace("_", " ").title() for t in p.get("types", [])
                ),
                "photo": photo_url,
            })

        # ===============================
        # RETURN BOTH: insight + places
        # ===============================
        return {
            "text": req.text,
